...
public class HbdpConnection {
	...
	private final String TAG = "HbdpConnection";
	...
	private class HbdpOutputStream extends OutputStream {
		...
		public synchronized void write(byte[] b, int offset, int length) throws IOException {
			if (exception != null) {
				Log.d(TAG, "throwing exception in write mtehod");
				throw exception;
			}
			if (closing)
			...
			Log.d(TAG, "starting to write to output");
			written += length;
			stateChanged();
		}
	...
	private class HbdpInputStream extends InputStream {
		...
		public synchronized int read(byte[] b, int offset, int length) throws IOException {
			if (exception != null) {
				Log.d(TAG, "throwing exception in read(byte[]...");
				throw exception;
			}

			Log.d(TAG, "starting read... (array method)");
			...
			while (readable == 0) {
				...
				try {
					Log.d(TAG, "starting to wait...");
					wait();
				} catch (InterruptedException e) {
					Log.d(TAG, "Exception in read(byte[] b... method");
		...
		public synchronized int read() throws IOException {
			if (exception != null) {
				Log.d(TAG, "throwing exception in read...");
				throw exception;
			}
			...
			while (readable == 0) {
				...
				try {
					...
					Log.d(TAG, "throwing IOException in read method");
					throw new IOException(e);
				}
				if (exception != null)
					throw exception;
	...
	private class HbdpThread extends Thread {
		...
		public void run() {
			...
			Log.d(TAG, "starting HbdpThread run");
			try {
				if (session == null)
					connect();
				loop();
			} catch (InterruptedException e) {
				Log.d(TAG, "throwing InterruptedException in run!");
				exceptionOccurred(new IOException(e));
			} catch (IOException e) {
				Log.d(TAG, "caught IOException in run");
				exceptionOccurred(e);
			}
		}
		...
		private void connect() throws IOException {
			...
			Log.d(TAG, "Connect method, attempting to connect");
			connection.connect();
			Log.d(TAG, "Connect method, connected");
			...
			Log.d(TAG, "connect method, disconnected");
